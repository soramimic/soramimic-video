...
昆虫のように「写真は見たくない」人がいる単語リストは、
`src/soramimic_video/wordlist_image_optout.json` に登録すると既定で画像を出さなくなる。

```json
{"insect": {"reason": "昆虫の写真が苦手な人がいるため、既定では画像を出しません",
            "layout": "noimage_card"}}
```

- `reason`: Web UIのチェックボックスの下に出す説明
- `layout`: 画像を隠しているあいだの既定レイアウト(省略可。組み込みレイアウト名)

見たい人は Web UI の単語リスト欄にある「画像を表示する」にチェックを入れる
(API は `POST /api/jobs` の `show_images=true`、CLI は `video --show-images`)。
チェックが無いあいだは**サムネも動画も画像なし**になり、画像のダウンロードや
クレジット収集も行わない(どのレイアウトを選んでいても同じ)。
