...
歌詞を入力した場合も、音源の自動認識と読み・音符の推定を行います。
その結果と入力歌詞を対応付け、確かな箇所の表示に入力表記を使います。
...
通常の自動解析を終えてから入力歌詞を対応付け、一対一で対応した行の読みを再確認します。
読みが変わる行だけを再整列し、他の行の歌唱データが変わる場合は変更を採用しません。
改行をまたぐ対応や、音声の根拠が曖昧・矛盾する箇所は自動解析の読みを維持します。
...
入力歌詞の原文・対応・未解決行と局所的な読み変更は`analyze_audio/lyric_surface.json`と
`project.json`の`lyric_layers.lyric_surface`に保持します。表記の修正だけでは音符や読みを変えません。
