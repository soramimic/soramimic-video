...
単語リストのプルダウンで「自作リストを使う(CSV)」を選ぶと、手元のテキスト/CSV
ファイルをアップロードしてそのまま変換に使える(用意されているリストに無いものに
空耳させたいとき用。以前あった「その他(名前を入力)」の代わり)。
...
ファイルを選んだ時点でサーバー(`POST /api/wordlist-check`)が列・行数・読みを検査し、
読めた語数を出すか、駄目なら理由を出してその場で取り消す。通ったCSVは投入時に
`POST /api/jobs` の `wordlist_csv` として送られ、**そのジョブのディレクトリにだけ**
保存されて変換に使われる(単語リストDBの共有キャッシュには載せない)。

制限と非対応:

- サイズ上限は 2MB / 10,000行。`SORAMIMIC_MAX_WORDLIST_BYTES` /
  `SORAMIMIC_MAX_WORDLIST_ROWS` で変えられる
- `image` / `image_page` 列は受け付けない(サーバーが任意のURLを取りに行かないため)。
  自作リストは**文字だけ**の動画になり、レイアウトはサーバー既定が使われる
- サムネのプレビュー・🎲ランダムの抽選・同梱editorでの替え歌編集は非対応
