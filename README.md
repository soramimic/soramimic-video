...
その場に出るサムネ枠をタップするだけで生成が始まる。カードにある操作はこの枠と、
右上の **🎲(曲と単語リストをランダムに選ぶ)** と **⚙(替え歌を編集)** の3つだけ。
単語リストの選び直し・絞り込み・変換のしかたの調整は、⚙ で開く同梱エディタの
⚙モーダル(「変換のしかた」)にまとめてある。枠の下の1行が、いま何が作られるか
(「かたつむり × 駅名」)を示す。
...
同梱すると、Web UI のカード右上の⚙(替え歌を編集)で替え歌変換 → その場の
