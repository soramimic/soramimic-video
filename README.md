...
```sh
git clone --recursive https://github.com/soramimic/soramimi-video.git
cd soramimi-video
uv sync                          # Python側
...
uv run soramimi-video analyze --midi song.mid --lyrics lyrics.txt --project work/song

# 2. 替え歌単語歌詞に変換(soramimic)
uv run soramimi-video convert --project work/song --wordlist stations

# 3. 人手編集(任意)
uv run soramimi-video export-edit --project work/song
#    work/song/edit.json を編集して…
uv run soramimi-video import-edit --project work/song

# 4. NEUTRINOで歌唱合成
NEUTRINO_ROOT=~/NEUTRINO uv run soramimi-video synthesize --project work/song --model MERROW

# 5. 伴奏(元MIDI、メロディ消音)とミックス
uv run soramimi-video mix --project work/song --soundfont /path/to/GeneralUser.sf2

# 6. 替え歌動画(単語リストの画像+元歌詞/替え歌字幕)
uv run soramimi-video video --project work/song
```
