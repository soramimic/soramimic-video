...
- `gen_samples.py` — 同梱サンプル曲(童謡・唱歌)のXF MIDIを生成するスクリプト。
  いずれも詞・曲ともパブリックドメインで、メロディは公知の楽譜の手打ち。
  出力は `src/soramimic_video/static/sample/` に置かれ、Web UIの
  「サンプル曲をセット」と `/api/samples`, `/api/sample/*` が配信する。
  曲ごとの権利根拠(作詞・作曲者の没年)は `docs/sample-rights.md` を参照
