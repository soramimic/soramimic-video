...
def test_resolve_granularity_precedence():
    # 要素の指定 > override > source既定。既定は各Project.Lineを独立させる
    assert resolve_granularity("original", None, None) == "cue"
    assert resolve_granularity("parody", None, None) == "cue"
...
def test_default_cue_granularity_does_not_merge_broken_xf_word_boundary():
    """語中に見える境界でも、字幕側ではProject.Lineを自動結合しない。"""
