...
def test_slow_wordlist_notice_only_appears_during_full_generation_conversion():
    """数分の注意はサムネ作成や結果下には出さず、本編の空耳変換中だけ出す。"""
    script = _script()
    ...
    assert "数分かか" not in preview
    assert "builder-loading-label" not in _markup()
