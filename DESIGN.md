...
既知歌詞の既定経路は、自動解析を先に完了し、入力表記を`lyric_surface`として対応付けます。
認識表記・読み・文字範囲を持つ`canonical`は表示だけの修正では変更しません。
`Line.original_text`と`original_line_index`は表示グループを参照し、複数の認識行が
同じ入力フレーズを共有する場合も、歌唱用の行や音符は統合しません。未対応の入力行は
原文とともに保持し、未歌唱とは断定しません。
一対一で対応した行の読み候補はKanaWhisperで原音・分離ボーカルと照合します。
読みの変更はその行内だけをCTCで再整列し、他の行の歌唱データを変更しない場合に限り採用します。
改行をまたぐ対応は読みを維持します。明示したルビは候補へ反映し、局所的な整列が
成立しなければ無理に適用せず、判断を`lyric_surface.reading_reviews`に残します。
