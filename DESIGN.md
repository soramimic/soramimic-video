...
ただし音源入力で`adjust_lyrics=true`を明示した場合は、音声認識と照合した後の
行を`canonical`とします。対応した入力行の表記は保ち、未歌唱行の削除・繰り返し・
新しい行の追加だけを行います。補正前の入力と判断は`lyric_adjustment`に保持します。
既定はfalseで、MIDI入力や入力歌詞なしでは指定できません。
既知歌詞の既定経路は、自動認識・暫定整列を位置探しの根拠にし、入力歌詞を対応付けます。
対応したまとまりごとに入力歌詞から読みを確定し、最後にCTC整列・音符対応付けを行います。
読み候補へ自動認識の別の言葉は追加しません。明示ルビは固定し、残る辞書候補の曖昧さだけを
KanaWhisperで比較します。証拠が曖昧なら入力歌詞側の既定読みを保ちます。
複数の認識行・入力行が対応する場合は一つの最終歌詞区間にまとめます。
`lyric_surface`の`asr_indices`は元の認識行、`line_indices`は最終歌詞行を指します。
`canonical`には入力表記と確定した読みを保持し、未対応の認識行だけ自動認識を使います。
未対応の入力行は原文とともに保持し、未歌唱とは断定しません。
最終整列が成立しなければ診断を残してエラーにし、自動認識の読みへ戻しません。
`Line.original_text`と`original_line_index`は最終的な表示グループを参照します。
`analysis.json`の`lyric_asr_used`と`reading_asr_used`は、歌詞本文の認識と読みの照合を区別します。
根拠のない省略や合成先が未解決のレイヤーは取り込み時にエラーにします。
VOICEVOXで短い音符をフレームへ丸める際は、同じ歌詞行の範囲内で長さを再配分して
全モーラを保持します。元のノートとレイヤーの推定時刻は変更しません。行内の長さが
エンジンの最小発音長を満たせない場合は、歌詞を削らずエラーにします。
