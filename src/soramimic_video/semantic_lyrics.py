...
def non_lyric_template_family(text: str) -> str | None:
    ...
    if not normalized:
        return None
    if _CREDIT_WITH_VALUE.fullmatch(unicodedata.normalize("NFKC", text)):
        return "credits"
...
def interval_has_melodic_support(
    ...
    """Treat any positive SheetSage overlap as support; absence alone never rejects."""
    return any(
        max(start_sec, note.start_sec) < min(end_sec, note.end_sec)
        for note in notes
    )
...
def decide_recognized_line(
    ...
    """Reject only the exact conjunction of no melody and a template-family match."""
    normalized = normalize_recognized_text(line.text)
    family = non_lyric_template_family(line.text)
    supported = interval_has_melodic_support(line.start_sec, line.end_sec, notes)
